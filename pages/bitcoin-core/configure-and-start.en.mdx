...
If you've setup UFW it's important to open port 8333. This makes sure other nodes in the network can connect with yours. Opening this port is only neccesary if you're \(also\) running your node on cleannet. If you're only running your node on Tor, you can skip this step and continue with "start".
...
Everytime a block is verified, you'll see a line appearing that says something like `progress=X.XXXXX`. This is a number from 0 to 1. Zero means nothing is synchonized, one means you're done synchronizing. With `Crtl + C` you can stop the live feed.
...
With this command you can get information about the netwerk. If everything is alright, the output will look similar to the one below if you only use Tor.
...
If you're running your node on both Tor and clearnet, you ouput will look similar to this:

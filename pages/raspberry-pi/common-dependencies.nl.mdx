...
Nadat de oude versie is verwijderd kunnen we de nieuwe installeren. Op moment van schrijven is [v1.23.2](https://golang.org/dl) het meest recent. Als er een nieuwe versie is die je wil installeren \(of omdat een latere versie van LND dat vereist\), let er dan op dat je een ARM64 versie van Go download voor je Raspberry Pi. Als je bijvoorbeeld op een Intel NUC draait, dan heb je de amd64 versie nodig.
...
Als het goed is zie je `go version go1.23.2 linux/arm64` verschijnen.
...
# Verwachte output: v22.10.0
```

## Yarn

Download en installeer [Yarn](https://classic.yarnpkg.com/lang/en/docs/install/#debian-stable).

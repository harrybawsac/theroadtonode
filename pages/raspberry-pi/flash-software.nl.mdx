...
Het is tijd om iets met een beeldscherm te doen. De Pi moet een besturingssysteem hebben, namelijk [Ubuntu Server 20 LTS](https://ubuntu.com/download/raspberry-pi). Ook [Raspberry Pi OS Lite](https://downloads.raspberrypi.org/raspios\_lite\_armhf\_latest) is nodig voor het eenmalig instellen van de Pi. Om de besturingssystemen op zowel de SSD als de microSD te krijgen, gebruiken we het programma [Balena Etcher](https://www.balena.io/etcher). Gebruik je Windows dan zou je ook [Rufus](https://github.com/pbatard/rufus/releases/download/v3.17/rufus-3.17.exe) kunnen gebruiken.

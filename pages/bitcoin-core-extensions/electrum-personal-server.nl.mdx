...
```bash
wget https://github.com/chris-belcher/electrum-personal-server/archive/eps-v0.2.3.tar.gz
tar -xvf eps-v0.2.3.tar.gz
rm eps-v0.2.3.tar.gz
```

Maak een kopie van het configuratie bestand en pas deze aan.

```bash
cp electrum-personal-server-eps-v0.2.3/config.ini_sample config.cfg
nano config.cfg
```
...
```bash
cd electrum-personal-server-eps-v0.2.3
pip3 install wheel
pip3 install use .
```

...
Voordat je het opslaat moet je een tweetal aanpassingen maken. Vul op de plekken `USERNAME` en `PASSWORD` de gebruikersnaam-wachtwoord combinatie die je hebt aangemaakt in het [authenticatie deel van de Bitcoin Core configuratie](/bitcoin-core/configuratie-en-starten). Zodra alles goed staat kun je het bestand opslaan met `Ctrl + X` en bevestigen met `Y`.

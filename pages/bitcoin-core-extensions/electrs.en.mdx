...
<Callout emoji="&#x2139;">
	Time: 30 minutes of work, then 12 hours or tor everything  to index.
</Callout>
...
Test if it works. The expected output is `v0.9.7`.
...
With `sudo systemctl status electrs` you can see if everything is running properly and get an output similar to the one below.
...
```bash
git checkout -f <OUTPUT FROM PREVIOUS STEP> # for example 0.9.7
```
...
Translated with www.DeepL.com/Translator (free version)import Callout from "nextra-theme-docs/callout";
